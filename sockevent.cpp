...
bool KqueueSockEvent::add_event(int fd, bool enable_read, bool enable_write)
{
	return set_event(fd, true, true);
}

bool KqueueSockEvent::set_event(int fd, bool enable_read, bool enable_write)
{
	struct kevent ke;
	...
	if (kevent(_event_handle, &ke, 1, NULL, 0, NULL) != 0) {
		printf("kevent SET fail fd:%d\n", fd);
		return false;
	}

	flags = enable_write ? EV_ENABLE : EV_DISABLE;
	memset(&ke, 0, sizeof(ke));
	EV_SET(&ke, fd, EVFILT_WRITE, EV_ADD | flags, 0, 0, NULL);
	if (kevent(_event_handle, &ke, 1, NULL, 0, NULL) != 0) {
		LOGW("kevent set_event fail fd:%d\n", fd);
		return false;
	}
...
bool KqueueSockEvent::remove_event(int fd)
{
	...
	if (kevent(_event_handle, &ke, 1, NULL, 0, NULL) != 0) {
		LOGW("kevent remove_event read fail fd:%d", fd);
	}

	EV_SET(&ke, fd, EVFILT_WRITE, EV_DELETE, 0, 0, NULL);
	if (kevent(_event_handle, &ke, 1, NULL, 0, NULL) != 0) {
		LOGW("kevent remove_event write fail fd:%d", fd);
	}
...
bool EpollSockEvent::add_event(int fd, bool enable_read, bool enable_write)
{
	...
	//_mutex.lock();
	bool rc = (epoll_ctl(_event_handle, EPOLL_CTL_ADD, fd, &ev) == 0);
	//_mutex.unlock();
	return rc;
}

bool EpollSockEvent::set_event(int fd, bool enable_read, bool enable_write)
{
	struct epoll_event ev;
	memset(&ev, 0, sizeof(ev));
	...
	//_mutex.lock();
	bool rc = (epoll_ctl(_event_handle, EPOLL_CTL_MOD, fd, &ev) == 0);

	return rc;
}

bool EpollSockEvent::remove_event(int fd)
{
	struct epoll_event ev;
	memset(&ev, 0, sizeof(ev));
	...
	ev.events = 0;
	//_mutex.lock();
	bool rc = (epoll_ctl(_event_handle, EPOLL_CTL_DEL, fd, &ev) == 0);
...
int EpollSockEvent::get_events(int timeout, IOEvent *ioevents, int cnt)
{
	...
	for (int i = 0; i < res; i++) {
		ioevents[i]._fd =  events[i].data.fd;
